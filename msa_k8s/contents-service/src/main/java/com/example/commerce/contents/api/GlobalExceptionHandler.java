...
/**
 * Contents Service에서 발생한 예외를 일관된 오류 응답으로 변환한다.
 */
...
public class GlobalExceptionHandler {
    ...
    /**
     * 커서와 쿼리 파라미터 검증 실패를 400 응답으로 변환한다.
     *
     * @param exception 잘못된 요청 예외
     * @param request HTTP 요청
     * @return 잘못된 요청 응답
     */
    @ExceptionHandler({ConstraintViolationException.class, InvalidCursorException.class})
    ...
    /**
     * 존재하지 않는 콘텐츠 조회를 404 응답으로 변환한다.
     *
     * @param exception 콘텐츠 미존재 예외
     * @param request HTTP 요청
     * @return 미존재 오류 응답
     */
    ...
    public ResponseEntity<ErrorResponse> handleNotFound(
            ...
            HttpServletRequest request) {
        return response(HttpStatus.NOT_FOUND, "CONTENT_NOT_FOUND", exception.getMessage(), request, Map.of());
    }

    /**
     * 버전 충돌과 허용되지 않은 상태 변경을 409 응답으로 변환한다.
     *
     * @param exception 콘텐츠 충돌 예외
     * @param request HTTP 요청
     * @return 충돌 오류 응답
     */
    @ExceptionHandler({ContentVersionConflictException.class, InvalidContentStateException.class})
    public ResponseEntity<ErrorResponse> handleConflict(
            RuntimeException exception,
            HttpServletRequest request) {
        return response(HttpStatus.CONFLICT, "CONTENT_CONFLICT", exception.getMessage(), request, Map.of());
    }
    ...
    public ResponseEntity<ErrorResponse> handleUnexpected(Exception exception, HttpServletRequest request) {
        log.error("처리하지 못한 콘텐츠 API 오류가 발생했습니다.", exception);
